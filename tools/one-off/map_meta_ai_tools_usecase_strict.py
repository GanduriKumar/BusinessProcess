...
OUTPUT_FILE = OUTPUT_DIR / "Meta_AI_Tools_Mapped_UseCase_Strict.xlsx"
...
def build_agent_text(agent: dict[str, str]) -> str:
    return normalize(" ".join([agent.get("Use Case", ""), agent.get("Use Case", ""), agent.get("Agent Name", "")]))
...
def best_match(tool_row: dict[str, str], agents: list[dict[str, str]], vectors, vectorizer: TfidfVectorizer) -> dict[str, str]:
    ...
    return {
        ...
        "Use Case Match Confidence": confidence(score),
    }
...
def main() -> None:
    ...
    for tool in tools:
        row = dict(tool)
        row.update(best_match(tool, agents, vectors, vectorizer))
        mapped.append(row)

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    workbook = xlsxwriter.Workbook(str(OUTPUT_FILE))
    write_sheet(workbook, "AI Tools", tools)
    write_sheet(workbook, "AI Tools UseCase Mapped", mapped)
    workbook.close()
    print(f"Created strict use-case workbook: {OUTPUT_FILE}")
